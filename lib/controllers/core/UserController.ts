...
export class UserController extends BaseController {
  ...
  public getModuleDescription(): string {
    return "Módulo para gerenciamento de perfil, autenticação de sessão e permissões de administrador.";
  }
 ...
 */
public async getBasicProfile(userId?: string) {
...
public async getUserProfile(userId?: string) {
  ...
  const [
    ...
    this.itemController.getUserItems(targetUserId),
    this.cardController.getUserCards(targetUserId),
    this.exchangeController.getCurrentExchange(targetUserId),
 ...
 */
public async makeAdmin(targetUserId: string) {
  const supabase = await this.getClient();
 ...
 */
public async removeAdmin(targetUserId: string) {
  const supabase = await this.getClient();
  const { data: { user } } = await supabase.auth.getUser();
  if (!user) return { error: "Não autenticado" };
