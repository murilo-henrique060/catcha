...
export class FriendController extends BaseController {
  ...
  public getModuleDescription(): string {
    return "Gerencia amizades e busca de jogadores públicos.";
  }
 ...
 */
public async acceptFriendRequest(senderId: string) {
  const supabase = await this.getClient();
  const { data: { user }, error: authError } = await supabase.auth.getUser();
  if (authError || !user) {
    return { error: "Usuário não autenticado" };
 ...
 * @returns Objeto indicando sucesso ou a mensagem de erro.
 */
public async declineFriendRequest(senderId: string) {
  const supabase = await this.getClient();
  const { data: { user }, error: authError } = await supabase.auth.getUser();
  if (authError || !user) {
    return { error: "Usuário não autenticado" };
