...
export class GiftController extends BaseController {
  ...
  public getModuleDescription(): string {
    return "Módulo para gerenciamento de presentes entre amigos.";
  }
 ...
 */
public async sendGift(friendId: string, catId: number) {
 ...
 */
public async receiveGift(giftId: string) {
  const supabase = await this.getClient();
  const user = await this.userController.getUserProfile();
 ...
 */
public async getGiftsHistory(profileId: string) {
  const supabase = await this.getClient();
